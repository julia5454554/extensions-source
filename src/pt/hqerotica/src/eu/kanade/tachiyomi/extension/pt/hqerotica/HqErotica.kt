package eu.kanade.tachiyomi.extension.pt.hqerotica
...
class HqErotica(
    ...
    override val name = "HqErotica"
    override val baseUrl = "https://hqerotica.com"
    ...
    override fun popularMangaParse(response: Response): MangasPage {
        ...
        val mangas = items.mapNotNull { item ->
            ...
            if (href.isEmpty() || href.contains("/page/")) return@mapNotNull null
    ...
    override fun mangaDetailsParse(response: Response): SManga {
        ...
        return SManga.create().apply {
            title = document.selectFirst("h1.post-titulo, h1.entry-title, h1")?.text()?.trim()
                ?: document.title()
            author = document.selectFirst(".post-itens li:contains(Artista:) a, .post-itens li:contains(Tradutor:) a")?.text() ?: ""
            genre = document.select("ul.post-itens a[rel='tag'], .post-itens li a[href*='/category/'], .post-itens li a[href*='/tag/']")
    ...
    override fun pageListParse(response: Response): List<Page> {
        ...
        // Seletores para capturar as imagens do capítulo
        val selectors = listOf(
            "ul.post-fotos img",          // principal (estrutura real)
