package eu.kanade.tachiyomi.extension.pt.quadrinhoseroticosblog
...
@Source
class QuadrinhosEroticosBlog(
    override val lang: String = "pt-BR",
    override val id: Long = 0L, // troque por um ID único na publicação
) : HttpSource() {

    override val name = "QuadrinhosEroticosBlog"
    override val baseUrl = "https://www.quadrinhoseroticos.blog"
    ...
    override fun popularMangaParse(response: Response): MangasPage {
        ...
        for (i in 0 until jsonArray.length()) {
            ...
            // Filtro de domínio
            if (!link.contains("quadrinhoseroticos.blog")) continue

            // Obtém o conteúdo HTML e parseia com Jsoup
            val contentHtml = post.getJSONObject("content").getString("rendered")
            val doc = Jsoup.parse(contentHtml)

            // Filtro de anúncio: precisa ter pelo menos 2 imagens no conteúdo
            val images = doc.select("img")
            if (images.size < 2) continue  // se tiver menos de 2 imagens, provavelmente é anúncio
