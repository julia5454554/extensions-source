...
class TheHentaiComics(
    ...
    override fun popularMangaParse(response: Response): MangasPage {
        ...
        for (i in 0 until jsonArray.length()) {
            ...
            // Filtro de domínio (opcional, mas recomendado para evitar anúncios)
            if (!link.contains("thehentaicomics.com")) continue

            // Obtém o conteúdo e parseia com Jsoup para filtrar anúncios
            val contentHtml = post.getJSONObject("content").getString("rendered")
            val doc = Jsoup.parse(contentHtml)
            val images = doc.select("img")

            // Filtro de anúncio: exige ao menos 2 imagens no conteúdo
